...
#if defined(TIM_CCER_CC1NE)
    isComplementaryChannel[channel - 1] = PY32_PIN_INVERTED(pinmap_function(pin, PinMap_TIM));
#endif
  }
}
...
uint32_t HardwareTimer::getTimerClkFreq()
{
   ...
   *   otherwise TIMxCLK = 4x PCLKx
   */
#if defined(AIRF4xx) || defined(AIRF7xx)
#if !defined(AIRF405xx) && !defined(AIRF415xx) &&\
    !defined(AIRF407xx) && !defined(AIRF417xx)
  RCC_PeriphCLKInitTypeDef PeriphClkConfig = {};
  HAL_RCCEx_GetPeriphCLKConfig(&PeriphClkConfig);
