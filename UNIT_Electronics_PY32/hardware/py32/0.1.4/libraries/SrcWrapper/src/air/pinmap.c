...
static void configure_dualpad_switch(PinName pin, int function, uint32_t LL_AnalogSwitch)
{
  ...
  if (((function & PY32_MODE_ANALOG) != PY32_MODE_ANALOG)
      && ((pin & PDUAL) == PDUAL)) {
    /**
...
void pin_function(PinName pin, int function)
{
  ...
  uint32_t mode  = PY32_PIN_FUNCTION(function);
  uint32_t afnum = PY32_PIN_AFNUM(function);
  uint32_t port = PY32_PORT(pin);
  ...
  if (PY32_PIN_ANALOG_CONTROL(function)) {
  ...
  if ((mode == PY32_PIN_OUTPUT) || (mode == PY32_PIN_ALTERNATE)) {
    if (PY32_PIN_OD(function)) {
  ...
  pin_PullConfig(gpio, ll_pin, PY32_PIN_PUPD(function));
